import React from 'react'
import { Link } from 'react-router-dom';
import moon from '../../../assets/moon.svg'
import sun from '../../../assets/sun.svg'
import { Flex } from '../../../uikit/uikit';
import { CalendarHeader, CalendarIconWrap, CalendarHeaderText, CalendarIcon } from '../CalenderSC';

type Props = {
    header: string[];
    value: any;
}

const CalendarList:React.FC<Props> = (props) => {   
  return (
    <>
        <CalendarHeader>
            {props.header.map((el,index) => {
                return <CalendarHeaderText key={index} style={{flex: index===0?'0.5':'1', margin: index===0?'0':'0 10px'}}>
                    {el}
                </CalendarHeaderText>
            })}
        </CalendarHeader> 
        <Flex direction='column' margin='0 0 100px 0'>
            {props.value.map((el:any,index) => {
                if(el !== undefined){
                    return <Flex key={index} margin='30px 0 0 0' align='center'>
                        <CalendarIconWrap><CalendarIcon src={el.icon=='moon'?moon:sun} alt="" /></CalendarIconWrap>
                        {el.value.map((el:any, index:number) => {
                            if(index === 0){
                                return <CalendarHeaderText key={index}>
                                    <Link to='#'>{el}</Link>
                                </CalendarHeaderText> 
                            }
                            return <CalendarHeaderText key={index}>{el}</CalendarHeaderText>
                        })}
                    </Flex>
                }
            })}
        </Flex>
        
    </>
  )
}

export default CalendarList